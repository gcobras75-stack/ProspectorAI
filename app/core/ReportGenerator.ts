...
function latLngToUTM(lat: number, lng: number): string {
  ...
  return `Z${zone} · ${Math.abs(lat).toFixed(4)}°${lat >= 0 ? 'N' : 'S'} ${Math.abs(lng).toFixed(4)}°${lng >= 0 ? 'E' : 'O'}`;
}
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .portada .meta-table td { padding: 8px 16px; font-size: 13px; border-bottom: 1px solid #333; }
  ...
  .page h3 { color: #333; font-size: 15px; font-weight: 700; margin: 20px 0 8px 0; }
  .page p { color: #444; font-size: 13px; line-height: 1.7; margin: 0 0 14px 0; }
  .page pre { color: #333; font-size: 12px; line-height: 1.8; white-space: pre-wrap; font-family: 'Courier New', monospace; background: #F9F9F9; padding: 12px; border-radius: 4px; border-left: 3px solid #FFD700; }

  /* ── Map ── */
  .map-img { width: 100%; max-height: 420px; object-fit: contain; border: 1px solid #DDD; border-radius: 4px; }
  .map-placeholder { width: 100%; height: 200px; background: #E8E8E8; display: flex; align-items: center; justify-content: center; border-radius: 4px; border: 1px dashed #BBB; }
  .map-placeholder p { color: #888; font-size: 13px; }
  .leyenda { display: flex; flex-wrap: wrap; gap: 16px; margin-top: 14px; padding: 10px 14px; background: #F5F5F5; border-radius: 6px; }
  .leyenda span { font-size: 11px; font-weight: 600; }
  .nota-mapa { color: #888; font-size: 10px; margin-top: 10px; font-style: italic; }

  /* ── Tables ── */
  .data-table, .points-table { width: 100%; border-collapse: collapse; font-size: 12px; margin-top: 10px; }
  .data-table th, .points-table th { background: #1A1A1A; color: #FFD700; padding: 8px 10px; text-align: left; font-size: 11px; text-transform: uppercase; letter-spacing: 0.5px; }
  .data-table td, .points-table td { padding: 7px 10px; border-bottom: 1px solid #EEEEEE; vertical-align: top; }
  .data-table tr:nth-child(even) td, .points-table tr:nth-child(even) td { background: #FAFAFA; }

  /* ── Dr. Ruiz ── */
  .section-label { color: #888; font-size: 11px; margin-bottom: 20px; font-style: italic; }
  ...
  .sample-photo-placeholder { width: 200px; min-width: 200px; height: 200px; background: #EEE; border-radius: 6px; display: flex; align-items: center; justify-content: center; border: 1px dashed #CCC; }
  .sample-info { flex: 1; }
  .sample-info h4 { color: #1A1A1A; font-size: 14px; font-weight: 700; margin: 0 0 6px 0; }
  .sample-info .sample-coord { color: #888; font-size: 10px; font-family: monospace; margin-bottom: 8px; }
  .sample-info .sample-resena { color: #444; font-size: 12px; line-height: 1.6; }
  .sample-info .sample-analisis { color: #666; font-size: 11px; margin-top: 8px; font-style: italic; }

  /* ── Disclaimer ── */
  .disclaimer p { color: #555; font-size: 12px; line-height: 1.8; }
  .disclaimer h2 { color: #999; }
</style>
`;

function buildSatelliteRows(satelitesSources: string, acquisitionDates: string): string {
  const sources = satelitesSources.split('·').map(s => s.trim()).filter(Boolean);
  const rows = sources.map(src => {
    let collection = '—';
    let resolution = '—';
    if (src.toLowerCase().includes('sentinel-2') || src.toLowerCase().includes('s2')) {
      collection = 'COPERNICUS/S2_SR_HARMONIZED';
      resolution = '10–20 m';
    } else if (src.toLowerCase().includes('aster')) {
      collection = 'ASTER/AST_L1T_003';
      resolution = '15–90 m';
    } else if (src.toLowerCase().includes('emit')) {
      collection = 'NASA/EMIT/L2A/RFL';
      resolution = '60 m';
    } else if (src.toLowerCase().includes('sentinel-1') || src.toLowerCase().includes('s1')) {
      collection = 'COPERNICUS/S1_GRD';
      resolution = '10 m';
    }
    return `<tr><td>${src}</td><td>${collection}</td><td>${resolution}</td><td>${acquisitionDates}</td></tr>`;
  });
  return rows.join('');
}

function levelBadge(point: any): string {
  const consensus = point.consensus_level || '';
  if (consensus === 'PRIORITY_TARGET') return '<span class="badge badge-priority">Prioritario</span>';
  if (consensus === 'TRIPLE_SPECTRAL') return '<span class="badge badge-triple">Triple</span>';
  if (consensus === 'CONFIRMED')       return '<span class="badge badge-confirmed">Confirmado</span>';
  return '<span class="badge badge-single">Individual</span>';
}

function buildTopPointsRows(analysisPoints: any[]): string {
  const top = analysisPoints
    .filter(p => p.score !== undefined || p.base_score !== undefined)
    .slice(0, 10);
  return top.map((p, i) => {
    const score = p.score ?? p.base_score ?? 0;
    const minerals = (p.indices_analizados || []).map((idx: any) => idx.nombre).slice(0, 3).join(', ') || '—';
    const evidencias = p.geologia_interpretada
      ? p.geologia_interpretada.substring(0, 80) + (p.geologia_interpretada.length > 80 ? '...' : '')
      : '—';
    return `<tr>
      <td><strong>${i + 1}</strong></td>
      <td style="font-family:monospace;font-size:10px">${latLngToUTM(p.lat, p.lng)}</td>
      <td>${levelBadge(p)}</td>
      <td style="font-size:11px">${evidencias}</td>
      <td style="font-size:11px">${minerals}</td>
    </tr>`;
  }).join('');
}
...
export async function generateAndShareReport(input: ReportInput): Promise<void> {
  ...
  if (!geologoTexto) {
    ...
    await saveReportContent(input.projectId, geologoTexto);
  }
...
<div class="page">
  ...
  <table class="data-table">
    ...
    ${buildSatelliteRows(input.satelitesSources, input.acquisitionDates)}
  </table>
...
<div class="page">
  ...
  <table class="points-table">
    ...
    ${buildTopPointsRows(input.analysisPoints)}
  </table>
</div>
