...
### `/api/structural/grid` (Sentinel-1 + DEM) — ARREGLADO EN `develop`/STAGING (2026-09-20), PENDIENTE de promover a master con OK humano
- **Estado:** commits `2e1d66e` (DEM: `ImageCollection(...).select('DEM').mosaic()` + proyección nativa) y `f32a517` (cruce DEM↔S1 por índice de punto) en `develop` del servidor. `regress.sh` en staging: 16 OK / 0 FAIL / 0 DIFF; el check 16 reemplaza a K1. Producción SIGUE ROTA hasta el merge.
- **Segundo bug hallado al verificar S1:** aun arreglado el DEM, `vv_mean`/`vv_texture` salían 0 en TODAS las celdas con 132–220 escenas S1: las celdas se cruzaban por coordenadas de `sampleRegions(geometries:true)` (centro de píxel: DEM 30 m ≠ S1 10 m) y nunca coincidían.
- **Abierto 1 — S1 no puntúa:** ningún consumidor usa `vv_mean`/`vv_texture` (solo están en la interfaz `StructuralCell`). `structuralScore` = `lineament_density` = Sobel del DEM; `near_lineament` = densidad > 0.4. La PWA además marca `fuentes.s1 = !!structuralResult`, así que mostrará "Sentinel-1/DEM" como fuente aunque S1 no entre al puntaje. Decidir: usar `vv_texture` en el score estructural, o etiquetar solo "DEM".
- **Abierto 2 — honestidad de datos:** una celda sin dato S1 devuelve `vv_mean: 0` / `vv_texture: 0` (0 dB sería una medición posible en teoría, aunque no ocurre en la práctica: es un relleno neutro, no una medición). Debería ser `null` (cambia la forma de la respuesta y el tipo `StructuralCell`).
- **Abierto 3 — efecto al promover:** la app nativa y la PWA empezarán a recibir estructura en el análisis profundo: aparecerán "Estructura ✓", `structuralScore` y `PRIORITY_TARGET` (S2+ASTER+estructura); los resultados cambian respecto a hoy. Validar `lineament_density` contra una referencia independiente (solo se comprobó plausibilidad de pendientes y de VV en dB).
- **Histórico (antes del arreglo):**
