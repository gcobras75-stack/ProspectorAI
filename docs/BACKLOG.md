...
## Auditoría de usabilidad — app-web/proyecto/[id].tsx + ValidationSheet.tsx (2026-09-21)
...
**Actualización (2026-09-23):** dos hallazgos pasaron de "pendiente" a aplicados por ser claridad aislada, no rediseño: (1) botón visible "Rectángulo" junto a "Trazar" (con "Salir" y pista "Arrastra el dedo sobre el mapa…" en la consola; ambos botones comparten `startTrace` con la misma confirmación de análisis sin guardar); (2) "Capa ON/OFF" → "☑/☐ Mapa de calor", y el chip de material limita su ancho al 46% para no chocar con la píldora. Verificados solo con `tsc`/`expo lint`, no en dispositivo. Sigue pendiente el conteo de toques (trazar → analizar).

## App nativa: brújula y "N" del mapa (2026-09-24) — prioridad baja, SIN implementar
Hallazgos de leer `app/(tabs)/index.tsx` (no probados en dispositivo); no bloquean nada.
1. **El ícono "N" (`index.tsx` ~1617, `styles.northIndicator`) es decorativo, no un botón:** es una `View` sin `onPress`, no reorienta nada. Su flecha gira `-mapRotation`, pero `mapRotation` solo se actualiza con `region.heading` en `onRegionChange` (~1433), y `react-native-maps` normalmente no entrega `heading` ahí (la `Region` trae solo lat/lng/deltas). En la práctica la "N" queda fija apuntando arriba. Como `rotateEnabled` no está en `false` (por defecto es `true`), si el usuario gira el mapa con dos dedos la "N" **miente** sobre dónde está el norte real. Las flechas cian (píldora de rumbo y marcador de ubicación) tampoco compensan ese giro: solo apuntan bien con el mapa norte-arriba. **Decisión pendiente:** (a) `rotateEnabled={false}` y "N" fija de referencia, o (b) leer el rumbo de la cámara (`getCamera().heading` / `onRegionChangeComplete`) y hacer de la "N" un botón que devuelva el mapa al norte.
2. **Bug real, "-1°":** `const trueHeading = heading ? heading.trueHeading || heading.magHeading : 0;` (~1410). Cuando el sensor no tiene rumbo verdadero, `trueHeading` vale `-1`, que en JS es *truthy*: nunca cae a `magHeading` y la píldora mostraría literalmente "-1°" (y las flechas cian rotarían -1°). Corrección propuesta: `heading ? (heading.trueHeading >= 0 ? heading.trueHeading : heading.magHeading) : 0`. El mismo patrón está en el HUD de navegación (~2097: `heading?.trueHeading ?? 0`, que con `-1` da un rumbo relativo desfasado 1°): revisarlo junto.
