...
El `.env` también define `EXPO_PUBLIC_CLAUDE_API_KEY` y `EXPO_PUBLIC_N2YO_API_KEY`: hoy **nada en el código las referencia**, así que no se hornean (verificado: 0 apariciones de `sk-ant`); si alguna vez se referencian, viajarían dentro del bundle público.
